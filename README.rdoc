...
* {deregistering domain perspectives}[file:features/deregister_domain_perspective.feature] when domain perspectives become irrelevant
* {listing service components}[file:features/listing_service_components.feature] when provisioning service components and mapping service components to domain perspectives
* {registering service components}[file:features/register_service_component.feature] when new service components are added
* {updating service components}[file:features/update_service_component.feature] when service component needs change
...
* {updating service definitions}[file:features/register_service.feature] when associating services with service components
* {deregistering service definitions}[file:features/deregister_service_definition.feature] when service and service component associations should no longer be made
* {associating service components with domain perspectives}[file:features/associations.feature] when modelling the domain.
* {changing associations}[file:features/associations_update.feature] when modelling the domain.
* {removing associations}[file:features/associations_remove.feature] when modelling the domain.
